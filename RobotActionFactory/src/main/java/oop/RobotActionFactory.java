...
public class RobotActionFactory {
        ...
        try {
            ...
            while (i < commands.length) {
                ...
                if (commands[i].startsWith("REPEAT")){
                    int times = Integer.parseInt(commands[i].substring(7,8));
                    i++;
                    SequenceOfActions actions = new SequenceOfActions();

                    while (!Objects.equals(commands[i], "END REPEAT")) {
                        addCommand(commands[i], actions);
                        i++;
                    }

                    Action repeat = new RepeatAction(times, actions);
                    sequence.add(repeat);

                    if(i+1 < commands.length) i++;
                    else{
                        return sequence;
                    }
                }

                addCommand(commands[i], sequence);
                i++;
            }
    ...
    public void addCommand (String command, SequenceOfActions sequence){
        switch (command){
            case "FORWARD" :
